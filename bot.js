...
const AIRTABLE_API_URL = `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE_ID}/Movies`;
...
bot.hears('PayPal', async (ctx) => {
  // Extract movieId from context or message text
  const movieId = /* logic to get movieId */;
  try {
    const movie = await getMovieById(movieId);
    if (movie) {
...
bot.hears('M-Pesa', async (ctx) => {
  ...
  try {
    const movie = await getMovieById(movieId);
    if (movie) {
      const phone = /* get user’s phone number */;
